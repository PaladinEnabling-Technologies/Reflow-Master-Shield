  ...
  ******************************************************************************************************************************************************************************/
  ...
  #include <avr/eeprom.h>                    // Includes the EEPROM library to save variables to EEPROM
  ...
  int Stage_Temperature_Setpoints[] =            { 150, 180, 210, 180, 150, 20, 10, 10, 10 };                                  //  Various Stage temperatures - Presetable
  ...
  byte PREH_Hysteresis;    //  Hysteresis value for PREHEAT = PREHEAT setpoint + PREH_Hysteresis. This value will trigger the count down timer /  
                           //  hold timer for the PREHEAT stage and prevent the elements from
  ...
  int Overall_Seconds = 0;
  int Overall_Previous_Second = 0;
  int Ramp_Rate = 0;
  
  unsigned long milliSeconds = 0;
  unsigned long DONE_Millis_Timer = 0;
  ...
  void Test_ENT_ESC_Counter_Value ()
  {
    ...
    // Can't use a external pull down resistor on this button, so you use the internal pull-up resistor and read for a 
    // digital low since when the button is not pushed, your getting in a digital high.
  ...
  void RunningCycleScreen()
  {
    ...
    int Diagnostic_Temp        =  0;
    int Diagnostic_Ramp_Rate   =  0;
    
    Diagnostic_Temp = Temp;
    Diagnostic_Ramp_Rate = Ramp_Rate;
    ...
    milliSeconds = millis();                                       //  Sets the milliSeconds timer to millis() so that we can calculate time.
    ...
    //*************************************************               Below the graph drawing functions        ********************************************************************
    
    GLCD.FillRect(38, 51,   1, 11);    // Bottom divider line - Left divider line
    GLCD.FillRect(51, 51,   1, 11);    // Bottom divider line - Middle divider line
    GLCD.FillRect(89, 51,   1, 11);    // Bottom divider line - Right divider line
    GLCD.DrawLine( 0, 51, 126, 51);    // Lyn om 'n blokkie te maak
  
    GLCD.FillRect( 54,  1,   1,  10);  // Top division rectangle in the middle
    GLCD.FillRect( 90,  1,   1,  10);  // Top division rectangle on the right hand side
    
    GLCD.FillRect(5, 53, 2, 8);        // Block on the left hand side of the status at the bottom
    GLCD.FillRect(32, 53,   1, 8);     // Blockk on the right hand side of the status at the bottom
    
    GLCD.DrawLine( 8,  53,  31,  53);  // Lyn bo die text om dit beter te laat lyk
    GLCD.SelectFont(System5x7, WHITE);
    GLCD.CursorToXY(8, 54);
    ...
    GLCD.CursorToXY(58, 3);  //40
  
    Temp_Printing(Stage_Temperature_Setpoints[0]);
  
    GLCD.DrawCircle(79, 4, 1); //61
    GLCD.CursorToXY(82, 3);  //64
    GLCD.print("C");
  
    //*************************************************
  
    GLCD.CursorToXY(95, 3);  //95
    Temp_Printing(Temp);
  
    GLCD.DrawCircle(116, 4, 1);         // Draws a little circle to be the Degree symbol
    GLCD.CursorToXY(119, 3);                                                                    // Print unit of measurent
    GLCD.print("C");
  
    //********************************************************                        Printing intial TIME                        *******************************************************************************
  
    GLCD.SelectFont(System5x7, BLACK);
    GLCD.CursorToXY(69, 54);
    GLCD.print(":");
  
    GLCD.DrawCircle(27, 4, 1);         // Draws a little circle to be the Degree symbol
    GLCD.CursorToXY(31, 3);
    GLCD.print("C/s");
    ...
    while ( Temp < Stage_Temperature_Setpoints[0] )
    {
      Setpoint = Stage_Temperature_Setpoints[0] + PREH_Hysteresis;
    ...
    if ( Temp >= Stage_Temperature_Setpoints[0] )
    {
      ...
      while ( ENT_Through_Status == 1)
      {
        ...
        if ( Minutes >= 0 && Seconds >= -1 && Second_Detector == 1 )   // Second detector used to check when a second has passed. So only once a second have passed is the seconds counter allowed
        {                                                             // decrement. Not at all related to Minutes or Seconds.
          Seconds--;
        ...
        {                 
          ...
          GLCD.CursorToXY(58, 3);
          Temp_Printing(Stage_Temperature_Setpoints[1]);
          ...
          if ( Temp >= Stage_Temperature_Setpoints[1] )
          {
            ENT_Through_Status++;
            ...
            StartofTimer_Seconds = millis();                              //  Set timers to millis again so that the timing can be accurate
            StartofTimer_Graphing_Interval = millis();
            ...
            while ( ENT_Through_Status == 2 )
            {
              ...
              while ( Minutes == 0 && Seconds <= 0 )
              {
                ...
                GLCD.CursorToXY(58, 3);
                Temp_Printing(Stage_Temperature_Setpoints[2]);
                ...
                if ( Temp >= Stage_Temperature_Setpoints[2] )
                {
                  ENT_Through_Status++;
                  Increment_Detector = 1;
                  Loop_Through_Detector = 1;
                  ...
                  while ( ENT_Through_Status == 3 )                    //    Solder Keep Stage - Pixal 74 is counted but not colored in.
                  {
                    ...
                    Ramp_Rate_Calc();      // Calculates the ramp rate of the temperature
                    Ramp_Printing();       // Displays the ramp rate of the temperature.
                    
                    Timing_Function();     // Prints the overall time of the process.
                    ...
                    if ( Minutes >= 0 && Seconds >= -1 && Second_Detector == 1 )    // Second_Detector is used to determine whether a second has passed and is implemented in the Temp_Constant_Graph_Validator
                    {
                      SLDR_Keep_SEC_Run--;
                    ...
                    if ( Minutes == 0 && Seconds <= 0 )
                    {
                      Increment_Detector = 1;
                      Loop_Through_Detector = 1;
                      DONE_Millis_Timer = millis();                    
                    }
  
                    while ( Minutes == 0 && Seconds <= 0 )
                    {
                      while ( Temp >= Stage_Temperature_Setpoints[3] )    //    Starting the Keep Phase of the process.
                      {                                                                                
                        Raw_Value = analogRead(sensorPin);
                        ...
                        GLCD.CursorToXY(58, 3);
                        Temp_Printing(Stage_Temperature_Setpoints[3]);
                        ...
                        if ( millis() >= ( DONE_Millis_Timer + 1000 ))
                        {
                          KEEP_SEC++;
                          DONE_Millis_Timer = millis();
                          ...
                          Ramp_Temp_Range = Stage_Temperature_Setpoints[3] - Stage_Temperature_Setpoints[2];   // We execute this code within the seoncd detector so that the code
                                                                                                               // is only executed every second. This is to limit a run away graph.
                          if ( Ramp_Temp_Range < 0 )
                          {
                            Ramp_Temp_Range = Ramp_Temp_Range * (-1);
                      ...
                      if ( Temp >= Stage_Temperature_Setpoints[4] )
                      {
                        Increment_Detector = 1;
                        DONE_Millis_Timer = millis();
                      }
  
                      while ( Temp >= Stage_Temperature_Setpoints[4] )          //    Entering the COOL stage of the process.
                      {                          
                        Raw_Value = analogRead(sensorPin);
                        ...
                        GLCD.CursorToXY(58, 3);
                        Temp_Printing(Stage_Temperature_Setpoints[4]);
                        ...
                        if ( Temp >= 0 && Temp < Stage_Temperature_Setpoints[4] )
                        {
                          Increment_Detector = 1;
                          DONE_Millis_Timer = millis();
                        }
  
                        while ( Temp >= 0 && Temp < Stage_Temperature_Setpoints[4] )      //  Entering the DONE stage of the process.
                        {
                          Raw_Value = analogRead(sensorPin);
                          ...
                          GLCD.CursorToXY(58, 3);
                          Temp_Printing(Stage_Temperature_Setpoints[5]);
                          ...
                          if (Temp <= Reflow_Program_Cutout_Temp || Minutes >= Reflow_Program_Cutout_Time )                          // Predefined cut-off time where unit exits back to the main menu again.
                          {
                            ...
                            Overall_Minutes = 0;                                                                                      // Sets the minutes back to zero so that the counter can re-initialize from 0 if the process is restarted.
                            Overall_Seconds = 0;                                                                                      // Sets the seconds back to zero so that the counter can re-initialize from 0 if the process is restarted.
  ...
  void Averaging_RAW_Value()
  {
    ...
    Raw_Value_Avg = total / numReadings;            // Raw_Value_Avg directly used to calculate Temperature.
    
    if ( Raw_Value_Avg >= 612 )                     //  If you limit Raw_Value_Avg to 612 then you limit the maximum temperature to 300 deg c since Temp = Raw_Value_Avg / 2.04
    {
      Raw_Value_Avg = 612;
    }
  }
  ...
  void Reference_Graph()
  {
    ...
    Temp_Limit1              =   0; //  These values are set back to their defaults so that the graph can plot correctly when the program is run again.
    Temp_Limit2              =  15;
    XY_Coordinates[0]        =   5; //  X coordinates
    XY_Coordinates[1]        =  48; //  Y coordinates
  ...
  {
    while ( Temp > Temp_Limit1 )                    //  Temp > Temp_Limit1; i.e. graph upwards
    {
      if ( Temp_Limit1 <= 150 );                    //  Temp_Limit1 <= 150
      {
        Temp_Step = 15 ;                            //  Temp_Step = 15 ;
      }
  
      if ( Temp > 150 && Temp_Limit1 >= 150)        //  Temp > 150 && Temp_Limit1 >= 150
      {
        Temp_Step = 5;                              //  Temp_Step = 5 ;
      }
  
      XY_Coordinates[1]--;                          //  Y coordinates, -- to graph Y upwards
  
      Temp_Limit1 = Temp_Limit1 + Temp_Step;
      Temp_Limit2 = Temp_Limit2 + Temp_Step;
  
      if ( XY_Coordinates[1] <= 15 )                // Y pixal limiter, limits the pixal at a minimum of the graph floor.
      {
        XY_Coordinates[1] = 15;
        Temp = 0;
      }
    ...
    while ( Temp < Temp_Limit1 )                    //  Temp > Temp_Limit1; i.e. graph upwards
    {
      if (  Temp <= 150 && Temp_Limit1 <= 150 );    //  Temp_Limit1 <= 150
      {
        Temp_Step = 15 ;                            //  Temp_Step = 15 ;
      }
      ...
      XY_Coordinates[1]++;                          //  Y coordinates, ++ to graph Y downwards
  
      Temp_Limit1 = Temp_Limit1 - Temp_Step;
      Temp_Limit2 = Temp_Limit2 - Temp_Step;
  
      if ( XY_Coordinates[1] >= 48 )                // Y pixal limiter, limits the pixal at a minimum of the graph ceiling.
      {
        XY_Coordinates[1] = 48;
        Temp = 260;
      }
  ...
  void Graphing_Function()
  {
    ...
    if ( XY_Coordinates[0] >= 120 )             // X graph limiter
    {
  ...
  {
    if ( Temp < Temp_Limit1 )                     //  if Temp < Temp_Limit1
    {
  
      if ( Temp_Limit1 <= 150 );                  //  if Temp_Limit1 <= 150
      {
        Temp_Step = 15 ;                          //  Temp_Step = 15 ;
      }
  
      if ( Temp > 150 && Temp_Limit1 >= 150)      //  if Temp >= 15- && Temp_Limit1 >= 150
      {
        Temp_Step = 5;                            //  Temp_Step = 5 ;
      }
      ...
      if ( XY_Coordinates[1] >= 48 )              //  Y graph limiter, limits Y at zero so that the graph doesn't plot below zero.
      {
        XY_Coordinates[1] = 48;
      }
  ...
  int Temp_Printing(int PrintTemp)
  {
    ...
    if ( PrintTemp <= 0 )
    {
      PrintTemp = 0;
      GLCD.print("  ");
      GLCD.print(PrintTemp);
    }
  
    if ( PrintTemp < 10 && PrintTemp > 0 )
    {
      GLCD.print("  ");
      GLCD.print(PrintTemp);
    }
  
    if ( PrintTemp < 100 && PrintTemp > 10  )
    {
      GLCD.print(" ");
      GLCD.print(PrintTemp);
    }
    ...
    if ( PrintTemp >= 100 )
    {
      GLCD.print(PrintTemp);                                                                                         // Print the Temp. reading
    }
  }
  ...
  void Ramp_Rate_Calc()
  {
    if ( Overall_Seconds == Overall_Previous_Second )
    {
      Temp1 = Temp;
    }
    ...
    if ( Overall_Seconds > Overall_Previous_Second )
    {
      ...
      if ( Overall_Seconds < 59 )
      {
        Overall_Previous_Second = Overall_Seconds;
      }
      else
      {
        Overall_Previous_Second = 0;
      }
    }
  
    Ramp_Rate = Temp1 - Temp2;  
  
    if ( Ramp_Rate < 0 )
    {
      Ramp_DWN_Pointer_Function();
      if (Ramp_Rate < -99)
      {
        Ramp_Rate = -99;
      }
    ...
    if ( Ramp_Rate > 0 )
    {
      Ramp_Up_Pointer_Function();
    }
    if ( Ramp_Rate == 0 )
    {
      Const_Ramp_Pointer_Function();
    }
  }
  ...
  void Timing_Function()
  {
    ...
    Convert_Overall_Seconds_to_Minutes();
  
    GLCD.CursorToXY(94, 54);
    Overall_Time_Printing(Overall_Minutes);
  
    GLCD.CursorToXY(107, 54);
    GLCD.print(":");
  
    GLCD.CursorToXY(113, 54);
    Overall_Time_Printing(Overall_Seconds);
  }
  //****************************************************************************************************************************************************************************
  //**************************************************                       Convert Seconds to Minutes                 ********************************************************
  //****************************************************************************************************************************************************************************
  
  void Convert_Overall_Seconds_to_Minutes()
  {
    if ( Overall_Seconds > 59 )
    {
      Overall_Minutes++;
  ...
  void Ramp_Printing()
  {
    ...
    if (Ramp_Rate < 0 && Ramp_Rate > -10)
    { 
      GLCD.print(" ");
      GLCD.print(Ramp_Rate);
    }
  ...
  void Ramp_Up_Pointer_Function()
  {
    GLCD.DrawLine( 45, 54,  45, 60);                  // Draw Up arrow in second block from left
    GLCD.DrawLine( 44, 55,  46, 55);
    GLCD.DrawLine( 43, 56,  47, 56);
  
    GLCD.FillRect( 43, 57,   1,  4, WHITE);
    GLCD.FillRect( 46, 57,   1,  4, WHITE);
  }
  
  void Ramp_DWN_Pointer_Function()
  {
    GLCD.DrawLine( 45, 54,  45, 60);                  //  Draw Down arrow in third block from left
    GLCD.DrawLine( 44, 59,  46, 59);
    GLCD.DrawLine( 43, 58,  47, 58);
  
    GLCD.FillRect( 43, 54,   1,  3, WHITE);
    GLCD.FillRect( 46, 54,   1,  3, WHITE);
  }
  
  void Const_Ramp_Pointer_Function()
  {
    GLCD.DrawLine( 43, 57,  47, 57  );
  
    GLCD.FillRect( 43, 54,   5,  2, WHITE);
    GLCD.FillRect( 43, 58 ,  5,  2, WHITE);
  }
  ...
  void Overall_Time_Printing( int Time_Variable)
  {
    if (Time_Variable < 10)
    {
      GLCD.print("0");
      GLCD.print(Time_Variable);
    }
